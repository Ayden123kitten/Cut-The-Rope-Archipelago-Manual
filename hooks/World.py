...
# region Event count patch
    fixed: set[str] = set()
    for location in multiworld.get_filled_locations(player):
        item = location.item
        if item is not None and item.code is None:
            # this if below is for future proofing so that if you update the Manual Core files
            # this patch wont break your logic later. If you have an event named the same as an item you'll have a false positive though
            if item.name not in fixed and item.name in world.item_counts_progression[player].keys():
                continue
            fixed.add(item.name)
            world.item_counts_progression[player][item.name] += 1
# endregion
    pass
